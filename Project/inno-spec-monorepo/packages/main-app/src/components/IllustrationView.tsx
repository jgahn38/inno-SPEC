...
import { Eye, Settings, Download, RotateCcw, AlertTriangle, CheckCircle } from 'lucide-react';
...
const IllustrationView: React.FC = () => {
  ...
  // const [detectedSection, setDetectedSection] = useState<any>(null); // 사용하지 않음
  ...
  // const [isSelectingLines, setIsSelectingLines] = useState(false); // 사용하지 않음
  const [cadViewMode, setCadViewMode] = useState<'2d' | '3d'>('2d');
  // const [cadZoom, setCadZoom] = useState(1); // 사용하지 않음
  // const [cadPan, setCadPan] = useState<[number, number]>([0, 0]); // 사용하지 않음
  
  // 레이어 선택 관련 상태
  const [showLayerSelection, setShowLayerSelection] = useState(false);
  const [availableLayers, setAvailableLayers] = useState<LayerInfo[]>([]);
  // const [selectedLayers, setSelectedLayers] = useState<string[]>([]); // 사용하지 않음
  ...
  const handleLayerSelectionConfirm = async (selectedLayerNames: string[]) => {
    ...
    // setSelectedLayers(selectedLayerNames); // 사용하지 않음
  ...
  const resetUpload = () => {
    ...
    setUploadError(null);
    setUploadSuccess(false);
    // setDetectedSection(null); // 사용하지 않음
    setCadData(null);
    setSelectedLines([]);
    setUploadProgress(0);
