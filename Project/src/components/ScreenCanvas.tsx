...
const ScreenCanvas: React.FC<ScreenCanvasProps> = ({
  ...
  const screenGridConfig = screen ? screen.gridConfig : null;
  ...
  const [originalComponents, setOriginalComponents] = useState<ScreenComponent[]>([]);
  const [originalLayout, setOriginalLayout] = useState<'single' | 'grid' | 'tabs'>('single');
  const [originalTabs, setOriginalTabs] = useState<Array<{ name: string; gridConfig: { rows: Array<{ cols: Array<{ width: number }> }> } }>>([]);
  ...
  const handleAddRow = (tabIndex: number) => {
    const newTabs = tabs.map((tab, i) => 
      i === tabIndex 
        ? { ...tab, gridConfig: { rows: [...tab.gridConfig.rows, { cols: [{ width: 0.25 }, { width: 0.25 }, { width: 0.25 }, { width: 0.25 }] }] } }
        : tab
    );
    setTabs(newTabs);
    ...
    const newTabs = tabs.map((tab, i) => 
      i === tabIndex 
        ? { 
            ...tab, 
            gridConfig: { 
              rows: tab.gridConfig.rows.filter((_, idx) => idx !== rowIndex) 
            } 
          }
        : tab
    );
  ...
  // 열 추가
  const handleAddCol = (tabIndex: number, rowIndex: number) => {
    const newTabs = tabs.map((tab, i) => 
      i === tabIndex 
        ? { 
            ...tab, 
            gridConfig: { 
              rows: tab.gridConfig.rows.map((row, idx) => 
                idx === rowIndex 
                  ? { cols: [...row.cols, { width: 0.25 }] }
                  : row
              )
            } 
          }
  ...
  // 열 삭제
  const handleDeleteCol = (tabIndex: number, rowIndex: number, colIndex: number) => {
    const newTabs = tabs.map((tab, i) => 
      i === tabIndex 
        ? { 
            ...tab, 
            gridConfig: { 
              rows: tab.gridConfig.rows.map((row, idx) => 
                idx === rowIndex 
                  ? { cols: row.cols.filter((_, colIdx) => colIdx !== colIndex) }
                  : row
              )
            } 
          }
  ...
  // 열 폭 비율 변경
  const handleColWidthChange = (tabIndex: number, rowIndex: number, colIndex: number, width: number) => {
    const newTabs = tabs.map((tab, i) => 
      i === tabIndex 
        ? { 
            ...tab, 
  ...
  const handleAddTab = () => {
    if (newTabName.trim()) {
      const newTabs = [...tabs, { 
        ...
        gridConfig: { rows: [{ cols: [{ width: 0.25 }, { width: 0.25 }, { width: 0.25 }, { width: 0.25 }] }, { cols: [{ width: 0.25 }, { width: 0.25 }, { width: 0.25 }, { width: 0.25 }] }, { cols: [{ width: 0.25 }, { width: 0.25 }, { width: 0.25 }, { width: 0.25 }] }] } 
      }];
      setTabs(newTabs);
      setNewTabName('');
      handleLayoutChange('tabs', undefined, newTabs.map(tab => tab.name));
  ...
  // 임시 탭 관련 함수들
  const handleTempAddRow = (tabIndex: number) => {
    const newTempTabs = tempTabs.map((tab, i) => 
      i === tabIndex 
        ? { 
            ...tab, 
            gridConfig: { 
  ...
  const handleTempDeleteRow = (tabIndex: number, rowIndex: number) => {
    const newTempTabs = tempTabs.map((tab, i) => 
      i === tabIndex 
        ? { 
            ...tab, 
  ...
  const handleTempAddCol = (tabIndex: number, rowIndex: number) => {
    const newTempTabs = tempTabs.map((tab, i) => 
      i === tabIndex 
        ? { 
            ...tab, 
  ...
  const handleTempDeleteCol = (tabIndex: number, rowIndex: number, colIndex: number) => {
    const newTempTabs = tempTabs.map((tab, i) => 
      i === tabIndex 
        ? { 
            ...tab, 
  ...
  const handleTempColWidthChange = (tabIndex: number, rowIndex: number, colIndex: number, width: number) => {
    const newTempTabs = tempTabs.map((tab, i) => 
      i === tabIndex 
        ? { 
            ...tab, 
  ...
  const handleTempAddTab = () => {
    const newTempTabs = [...tempTabs, { 
      name: newTabName || `탭 ${tempTabs.length + 1}`, 
      gridConfig: { 
        rows: [{ cols: [{ width: 0.25 }, { width: 0.25 }, { width: 0.25 }, { width: 0.25 }] }, { cols: [{ width: 0.25 }, { width: 0.25 }, { width: 0.25 }, { width: 0.25 }] }, { cols: [{ width: 0.25 }, { width: 0.25 }, { width: 0.25 }, { width: 0.25 }] }] 
      } 
    }];
    setTempTabs(newTempTabs);
    setNewTabName('');
  ...
  const handleTempDeleteTab = (index: number) => {
    const newTempTabs = tempTabs.filter((_, i) => i !== index);
    setTempTabs(newTempTabs);
  };

  const handleTempTabNameChange = (index: number, newName: string) => {
    const newTempTabs = tempTabs.map((tab, i) => 
      i === index ? { ...tab, name: newName } : tab
    );
    setTempTabs(newTempTabs);
